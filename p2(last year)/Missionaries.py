...
the formulation:
__eq__, __hash__, __str__, and the implicit constructor State().
...
class State:
    ...
    def can_move(self, m, c):
        # m stands for the number of missionary you actually want to carry.
        # for example, there is a move that let you carry 3 m, so m is 3
        # but if there is actually only 2 m on the boat side, then this move is not legal at this moment
        """Tests whether it's legal to move the boat and take m missionaries and c cannibals."""
        if m < 1:
            return False  # Need an M to steer boat.
    ...
    def __eq__(self, s2):
        if s2 is None:
            return False
        if self.n_boats_on_right != s2.n_boats_on_right:
            return False
        if self.n_missionaries_on_right != s2.n_missionaries_on_right:
            return False
        if self.n_cannibals_on_right != s2.n_cannibals_on_right:
            return False
        return True
