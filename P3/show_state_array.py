    ...
    for r in self.color_array:
      ...
      for c in r:
        ...
        if self.string_array:
          self.string_item_array[i][j]=STATE_WINDOW.canvas.create_text(x0+(j+0.5)*cellw, y0+(i+0.5)*cellh,
  ...
  def show(self):
    ...
    for r in self.color_array:
      ...
      for c in r:
        ...
        if self.string_array:
          STATE_WINDOW.canvas.itemconfig(self.string_item_array[i][j],text=self.string_array[i][j],fill=self.text_color,font=self.text_font)
        j += 1
      i += 1
    STATE_WINDOW.label.config(text=self.caption)
