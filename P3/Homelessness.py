...
class State:
    ...
    def move(self, money_adjustment, housing_price_adjustment, health_points_adjustment,
             employment_rate_adjustment, popularity_adjustment, homeless_people_adjustment,uncertainty=1.0,revenue_factor_adjustment=0.0,homeless_change_rate_adjustment=0):
        news = State(old=self)
        ...
        news.housing_price += news.housing_price*random.uniform(-housing_price_uncertainty,housing_price_uncertainty)*uncertainty
        news.health_points += health_points_adjustment*self.effectiveness_factor
        news.health_points += news.health_points*random.uniform(-health_points_uncertainty,health_points_uncertainty)*uncertainty
        news.employment_rate += employment_rate_adjustment*self.effectiveness_factor
        news.employment_rate += news.employment_rate*random.uniform(-employment_rate_uncertainty,employment_rate_uncertainty)*uncertainty
        news.popularity += popularity_adjustment*self.effectiveness_factor
        news.popularity += news.popularity*random.uniform(-popularity_uncertainty,popularity_uncertainty)*uncertainty
        ...
        news.homeless_people += news.homeless_people*random.uniform(-homeless_uncertainty,homeless_uncertainty)*uncertainty
...
class Operator:
    ...
    def apply(self, s):
        s.effectiveness_factor=self.diminishing_utility_factor**-Operator.apply_count_table[self.name] # reversed exponential fuction
